...
def test(args):
    ...
    def evaluate(data_loader):
        ...
        speaker_is_child = []
        with torch.no_grad():
            for batch_id, (input_samples, targets, sequence_lengths, age) in enumerate(
                data_loader
            ):
                # Move data to GPU
                ...
                if args.verbose:
                    for i, (sample, label, predicted) in enumerate(
                        zip(input_samples, targets, predicted_labels)
                    ):
                        if (
                            label_vocab.inverse[int(predicted)]
                            != label_vocab.inverse[int(label)]
                        ):
                            print(
        ...
        acc = int(
            sum([p == l for p, l in zip(all_predicted_labels, all_true_labels)])
        ) / len(all_true_labels)
        print("=" * 89)
        print("Test acc: {:5.2f}".format(acc))

        predicted_labels_adult = [
            label
        ...
        true_labels_adult = [
            label
            for label, is_child in zip(all_true_labels, speaker_is_child)
            if not is_child
        ]
        acc = np.equal(predicted_labels_adult, true_labels_adult).sum() / len(
            true_labels_adult
        )
        ...
        acc = np.equal(predicted_labels_child, true_labels_child).sum() / len(
            true_labels_child
        )
        ...
        if args.verbose:
            ...
            cm = confusion_matrix(
                ...
                predicted_labels,
                normalize="true",
                labels=list(label_vocab.values()),
            )

            for label_idx in label_vocab.inverse.keys():
                confusions = [
                    label_vocab.inverse[i]
                    for i in np.where(cm.T[label_idx] > 0.1)[0]
                    if i != label_idx
                ]
